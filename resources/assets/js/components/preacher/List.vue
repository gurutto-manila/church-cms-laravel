<template>
    <div>
        <portal to="preacher_count">
            <div class="">
                <h1 class="admin-h1 my-3">Preachers ( {{ Object.keys(this.users).length }} )</h1>
            </div>
        </portal>
                        ...
                        <div class="flex p-2 member-list active">
                            ...
                            <div class="px-2">
                                <h2 class="font-bold text-base text-gray-700">
                                    <a :href="url+'/admin/preacher/show/'+user['name']">{{ user.fullname }}</a>
                                </h2>
                                <p>{{ user.mobile_no }}</p>
                            </div>
                        </div>
...
<script>
    ...
    export default {
        ...
        created() 
        {
            axios.get('/admin/preachers/find?'+this.searchquery).then(response => {
        ...
        methods:
        {
            ...
            getUrl()
            {
                this.currenturl =  this.url+'/admin/preachers'; 
                if(this.searchquery!='')
                {
