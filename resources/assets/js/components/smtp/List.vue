        ...
        <div class="flex flex-row justify-between custom-table overflow-x-auto">
            <table class="w-full">
                <thead class="bg-grey-light">
                    <tr class="border-b">
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Host</th>
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Port</th>
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Username</th>
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Password</th>
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Encryption</th>
                ...
                <tbody class="bg-grey-light"  v-if="Object.keys(this.smtps).length > 0">
                    <tr class="border-b" v-for="smtp in smtps" v-bind:key="smtp.id">
                        ...
                        <td class="py-3 px-2">{{ smtp.encryption }}</td>
                        <td class="px-3 py-2">
                            <p v-if="smtp.status == 1" class="font-semibold text-xs">
...
<script>
    export default {
        ...
        methods:
        {
            getData()
            {
                axios.get('/'+this.mode+'/smtp/list').then(response => {
                    this.smtps = response.data.data;
                    //console.log(this.smtps);
                });
            },
            ...
            deleteSmtp(id)
            {
                ...
                swal({
                    ...
                    if (isConfirm)
                    {
                        axios.get(thisswal.url+'/'+thisswal.mode+'/smtp/delete/'+ id).then(response => {
