...
<script>
    ...
    export default {
        ...
        methods:
        {
            getData()
            {
                axios.get('/admin/faq/editList/'+this.id).then(response => {
            ...
            submitForm()
            {
                ...
                axios.post('/admin/faq/edit/'+this.id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {     
                    this.success = response.data.success;
                }).catch(error => {
                    this.errors = error.response.data.errors;
                });
