<template>
	<div>
		...
		<div class="flex flex-col lg:flex-row">
   			<div class="flex flex-wrap w-full lg:w-full lg:mr-3 my-3">
				        ...
				        <div class="relative flex items-center justify-end" v-if="mode == 'admin'">
				            ...
				            <div :id="id" class="hidden absolute bg-white w-20 shadow right-0 top-0 my-6 rounded">
				                <div class="flex flex-col text-xs  my-1">
				                    <a :href="url+'/'+mode+'/page/edit/'+page.id" class="capitalize text-gray-700 rounded px-4 py-1 mr-2 font-medium" target="_blank">edit</a>
		...
		<path d="M512,304c0-12.821-5.077-24.768-13.888-33.579c9.963-10.901,15.04-25.515,13.653-40.725
			...
			c15.765-7.936,26.368-24.171,26.368-42.688c0-7.552-1.728-14.784-5.013-21.333C501.419,338.731,512,322.496,512,304z"/>
		          			...
		          			<div class="text-sm">
		          				...
		          				<a href="#" class="no-underline text-white px-4 mx-1 flex items-center custom-green justify-center rounded" v-if="page.is_following == 1" @click="following(id,0)" title="Following">Following</a>
		          			</div>

		          			<div class="">
		          				<a :href="url+'/'+mode+'/post/add?entity_name='+entity_name+'&entity_id='+entity_id" class="no-underline text-white px-4 mx-1 flex items-center custom-green justify-center rounded" target="_blank" title="Create Post">
		          					<span class="mx-1 text-sm">Create Post</span>
...
<script>
	export default{
		...
		methods:
		{
			getData()
			{
				axios.get(this.url+'/'+this.mode+'/page/showList/'+this.id).then(response => {
            ...
            deletePage(id)
            {
                ...
                swal({
                    ...
                    if (isConfirm) 
                    {
                      axios.get(thisswal.url+'/'+thisswal.mode+ '/page/delete/'+ id).then(response => {
                        thisswal.success = response.data.success;
                        window.location = thisswal.url+'/'+thisswal.mode+ '/pages';
                      }); 
                    }
                    else 
                    {
			...
			following(id,val)
			{
		        ...
		        axios.post(this.url+'/'+this.mode+'/page/follow/'+id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
			...
			like(id,val)
			{
		        ...
		        axios.post(this.url+'/'+this.mode+'/page/like/'+id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
		          	this.success = response.data.success;
		          	window.location.reload();
		        }).catch(error => {
		          	this.errors = error.response.data.errors;
			...
			dislike(id,val)
			{
		        ...
		        axios.post(this.url+'/'+this.mode+'/page/dislike/'+id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
		          	this.success = response.data.success;
		          	window.location.reload();
		        }).catch(error => {
		          	this.errors = error.response.data.errors;
