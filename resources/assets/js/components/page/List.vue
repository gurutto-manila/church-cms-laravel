<template>
	<div class="my-1">
		...
		<div class="flex flex-wrap">
   			<div class="flex flex-wrap w-full lg:w-1/4 md:w-1/2 md:pr-2 lg:pr-3 my-2" v-for="page in pages">
      			<div class="bg-white shadow border border-grey px-5 py-3 w-full">
	   				<div class="flex justify-between items-center mb-2">
				        ...
				        <div class="relative flex items-center w-1/4 md:w-1/2 justify-end">
				            ...
				            <div :id="page.id" class="hidden absolute right-0 bg-white shadow right-0 top-0 my-5 rounded">
				                <div class="flex flex-col text-xs w-20 my-1">
				                    <a :href="url+'/'+mode+'/page/show/'+page.id" class="capitalize text-gray-700 rounded px-4 py-1 mr-2 font-medium" target="_blank">view</a>
         			...
         			<div class="">
		            	<img class="card-img-top h-40 w-40 md:h-40 lg:w-full" :src="page.cover_image">
		        	</div> 
		        	<p class="text-sm text-gray-700 flex items-center py-2" v-html="trim(page.description)">
		            	{{ page.description }}
		          	</p>
	  		...
	  		<div class="bg-white shadow p-4 w-full" v-if="Object.keys(pages).length == 0">
	  			<p class="flex items-center text-sm">No Records Found</p>
	  		</div>
		</div>
	</div>
</template>
...
<script>
	export default{
		...
		methods:
		{
			getData()
			{
				axios.get(this.url+'/'+this.mode+'/page/list').then(response => {
		    ...
		    trim(string) 
		    {
                var str = jQuery.parseHTML(string)[0]['innerText'];
