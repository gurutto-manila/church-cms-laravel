            ...
            <table class="w-full">
                <thead class="bg-grey-light">
                    <tr class="border-b">
                        ...
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Day After</th>
                        <th width="5%" class="text-left text-sm px-2 py-2 text-grey-darker" style="width:10%;">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-grey-light"  v-if="Object.keys(this.rules).length > 0">
                    <tr class="border-b" v-for="rule in rules" v-bind:key="rule.id">
                        <td class="py-3 px-2">{{ rule.name }}</td>
                        <td class="px-3 py-2">{{ rule.campaign_name }}</td>
                        <td class="px-3 py-2">
                            <p v-if="rule.status == 1" class="font-semibold text-xs">
                        ...
                        <td class="px-3 py-2">{{ rule.day_after }}</td>
                        <td class="py-3 px-2">
                            <div class="flex items-center">
                                <a :href="url+'/'+mode+'/rule/edit/'+rule.id" title="Edit">
                                    <img :src="url+'/uploads/icons/edit1.svg'" class="w-4 h-4 fill-current text-black-500 mx-1">
                                </a>

                                <a :href="url+'/'+mode+'/rule/show/'+rule.id" title="Show">
                                    <img :src="url+'/uploads/icons/show.svg'" class="w-4 h-4 fill-current text-black-500 mx-1">
                                </a>

                                <a href="#" @click="deleteCampaign(rule.id)" class="delete" title="Delete">
...
<script>
    export default {
        ...
        methods:
        {
            getData()
            {
                axios.get('/'+this.mode+'/rule/list').then(response => {
            ...
            deleteCampaign(id)
            {
                ...
                swal({
                    ...
                    if (isConfirm)
                    {
                        axios.get(thisswal.url+'/admin/rule/delete/'+ id).then(response => {
