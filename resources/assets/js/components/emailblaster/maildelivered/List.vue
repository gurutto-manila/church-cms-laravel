            ...
            <table class="w-full">
                <thead class="bg-grey-light">
                    <tr class="border-b">
                        ...
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Mailinglist</th>
                        ...
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Fired At</th>
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Read</th>
                        <th class="text-left text-sm px-2 py-2 text-grey-darker">Clicks</th>
                        <th width="5%" class="text-left text-sm px-2 py-2 text-grey-darker" style="width:10%;">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-grey-light"  v-if="Object.keys(this.mails).length > 0">
                    <tr class="border-b" v-for="mail in mails" v-bind:key="mail.id">
                        <td class="py-3 px-2">{{ mail.email }}</td>
                        <td class="py-3 px-2">{{ mail.subscriber_email }}</td>
                        <td class="py-3 px-2">{{ mail.mailinglist }}</td>
                        <td class="px-3 py-2">{{ mail.campaign }}</td>
                        <td class="py-3 px-2">{{ mail.scheduled_at }}</td>
                        <td class="py-3 px-2">{{ mail.fired_at }}</td>
                        <td class="px-3 py-2">
                            <p v-if="mail.is_read == 1" class="font-semibold text-xs">
                        ...
                        <td class="py-3 px-2">{{ mail.clicks }}</td>
                        <td class="py-3 px-2">
                            <div class="flex items-center">
                                <a :href="url+'/'+mode+'/mail-delivered/show/'+mail.id" title="Show">
...
<script>
    export default {
        ...
        methods:
        {
            getData()
            {
                axios.get('/'+this.mode+'/mail-delivered/list').then(response => {
