    ...
    <div class="bg-white p-3 shadow-xs my-3">
        ...
        <div class="flex flex-row justify-between custom-table overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="bg-grey-light"  v-if="Object.keys(this.webhooks).length > 0">
                    <tr class="border-b" v-for="webhook in webhooks" v-bind:key="webhook.id">
                        <td class="py-3 px-2">{{ webhook.name }}</td>
                        <td class="px-3 py-2">{{ webhook.mailinglist }}</td>
                        <td class="px-3 py-2">
                            <p v-if="webhook.status == 1" class="font-semibold text-xs">
                        ...
                        <td class="py-3 px-2">
                            <div class="flex items-center">
                                <a :href="url+'/'+mode+'/webhook/edit/'+webhook.id" title="Edit">
                                    <img :src="url+'/uploads/icons/edit1.svg'" class="w-4 h-4 fill-current text-black-500 mx-1">
                                </a>

                                <a :href="url+'/'+mode+'/webhook/show/'+webhook.id" title="Show">
                                    <img :src="url+'/uploads/icons/show.svg'" class="w-4 h-4 fill-current text-black-500 mx-1">
                                </a>

                                <a href="#" @click="deleteCampaign(webhook.id)" class="delete" title="Delete">
...
<script>
    export default {
        ...
        methods:
        {
            getData()
            {
                axios.get('/'+this.mode+'/webhook/list').then(response => {
            ...
            deleteCampaign(id)
            {
                ...
                swal({
                    title: 'Are you sure',
                    text: 'Do you want to delete this Webhook ?',
                    icon: "info",
                    buttons: [
                        'No',
                        'Yes'
                    ...
                    if (isConfirm)
                    {
                        axios.get(thisswal.url+'/admin/webhook/delete/'+ id).then(response => {
