<template>
    <div class="my-1">
        ...
        <div class="my-2">
            <div class="bg-white shadow p-5  py-4">
                <div class="">
                    <div class="flex items-center justify-between relative">
                         <div class="">
                        <p class="text-gray-800 text-xl capitalize">{{ post.title }}</p>
                    </div>   
                    <div class="flex items-center">
                        ...
                        <div :id="id" class="hidden absolute right-0 bg-white shadow top-0 rounded my-6" v-if="post.created_by == auth_id">
                            <div class="flex flex-col text-xs w-20 my-1">
                                <a :href="url+'/'+mode+'/post/edit/'+post.id" class="capitalize text-gray-700 rounded px-4 py-1 mr-2 font-medium my-1 lg:my-0 md:my-0" target="_blank">edit</a>
                ...
                <div class="flex text-xs justify-between flex-col lg:flex-row w-4/5">
                    <div class="my-2 lg:my-0 py-2">
                        <p class="text-gray-800 text-sm" v-html="post.description">{{ post.description }}</p>
                    </div>   
                </div>
                <div class="flex my-3 flex-wrap" v-if="post.attachments != null">
                    <div class="leading-loose  lg:w-1/6 md:w-1/6 p-1" v-for="attachment in post.attachments">
                        <img :src="attachment.path" class="lg:w-full md:w-32 w-32 lg:h-40 md:h-32 h-32"> <!-- w-20  -->
                    </div>
                </div>
                        ...
                        <div class="mr-6">
                            <div class="flex flex-wrap">
                                ...
                                <a href="#" @click="like(0)" v-if="post.like == 1" title="Liked">
                                    <svg class="w-5 h-5 text-teal-400 fill-current" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
<script>
    ...
    export default{
        ...
        methods:
        {
            getData()
            {
                axios.get(this.url+'/'+this.mode+'/post/showList/'+this.id).then(response => {
            ...
            like(val)
            {
                ...
                axios.post(this.url+'/'+this.mode+'/post/like/'+this.id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
            ...
            dislike(val)
            {
                ...
                axios.post(this.url+'/'+this.mode+'/post/dislike/'+this.id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
                    this.success = response.data.success;
                    window.location.reload();
                }).catch(error => {
                    this.errors = error.response.data.errors;
            ...
            save(val)
            {
                ...
                axios.post(this.url+'/'+this.mode+'/post/save/'+this.id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
                    this.success = response.data.success;
                    window.location.reload();
                }).catch(error => {
                    this.errors = error.response.data.errors;
            ...
            unsave(val)
            {
                ...
                axios.post(this.url+'/'+this.mode+'/post/unsave/'+this.id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
                    this.success = response.data.success;
                    window.location.reload();
                }).catch(error => {
                    this.errors = error.response.data.errors;
            ...
            deletePost(id)
            {
                ...
                swal({
                    ...
                    if (isConfirm) 
                    {
                      axios.get(thisswal.url+'/'+thisswal.mode+ '/post/delete/'+ id).then(response => {
                        thisswal.success = response.data.success;
                        //window.location = thisswal.url+'/'+thisswal.mode+ '/posts';
                      }); 
                    }
                    else 
                    {
