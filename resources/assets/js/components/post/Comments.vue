			    		...
			    		<div class="py-2 bg-white shadow border-b-8 p-5 w-full" v-for="reply in commentlist">
			    			<div class="flex flex-col lg:flex-row lg:items-center justify-between">
			    				<div class="flex flex-wrap my-2">
			    					...
			    					<h1 class="px-3 text-sm">{{ reply.user_fullname }}</h1>
			    				</div>
				    			<div class="bg-white flex items-center justify-center cursor-pointer relative">
							    	<div class="relative flex items-center w-full lg:justify-end px-1 lg:px-4 py-1 lg:py-1 md:py-1">
...
<g>
	<g>
		<path d="M512,304c0-12.821-5.077-24.768-13.888-33.579c9.963-10.901,15.04-25.515,13.653-40.725
			...
			<div class="flex flex-wrap my-2 w-full lg:w-3/4 items-center">
			          	...
			          	<div class="py-2 bg-white shadow border-b-8 p-5 w-full" v-for="comment in reply_comments_list">
			    			<div class="flex flex-col lg:flex-row lg:items-center">
			    				<div class="flex flex-wrap">
			    					...
			    					<h1 class="px-3 text-sm">{{ comment.user_fullname }}</h1>
			    				</div>
						          	...
						          	<div class="mr-6">
						          		<div class="flex flex-wrap">
						          			...
						          			<a href="#" @click="dislike(0,comment.id)" v-if="comment.unlike == 1" title="Disliked">
						          				<svg class="w-4 h-4 fill-current text-red-500" version="1.0" xmlns="http://www.w3.org/2000/svg" width="215.000000pt" height="234.000000pt" viewBox="0 0 215.000000 234.000000" preserveAspectRatio="xMidYMid meet"><g transform="translate(0.000000,234.000000) scale(0.100000,-0.100000)" fill="" stroke="none"><path d="M1160 2219 c-97 -12 -144 -25 -269 -76 -75 -30 -151 -52 -219 -63 -57 -10 -109 -23 -116 -30 -12 -12 -22 -973 -11 -1020 4 -13 73 -89 154 -170 241 -236 367 -402 483 -635 75 -149 88 -165 143 -165 112 0 207 143 207 312 0 130 -44 243 -146 379 -31 40 -52 77 -48 80 4 4 50 3 102 -3 149 -15 413 -5 482 19 137 48 172 112 131 244 -22 72 -20 82 22 143 61 89 60 165 -5 239 -22 25 -40 50 -40 55 0 6 13 39 30 74 46 97 38 156 -31 236 -33 38 -35 54 -13 129 15 51 15 58 -1 99 -28 73 -116 122 -271 149 -95 16 -467 19 -584 4z"/><path d="M52 2174 l-22 -15 0 -585 c0 -384 4 -592 10 -605 10 -17 22 -19 148 -19 94 0 142 4 150 12 16 16 17 1179 2 1209 -10 17 -22 19 -138 19 -99 0 -132 -3 -150 -16z"/></g></svg>
						          			</a>
    ...
    export default {
        ...
        methods:
        {
			...
			showComment(comment_id)
			{
				...
				axios.get(this.url+'/'+this.mode+'/post/replyComment/'+comment_id).then(response => {
			...
			addAttachment(type,event,comment_id = null)
			{
	      	...
	      	submitComment()
	      	{
	      		...
	      		axios.post(this.url+'/'+this.mode+'/post/add/comment/'+this.id,formData,{headers: {'Content-Type' : 'multipart/form-data'}}).then(response =>{
	      	...
	      	replyComment(post_id,comment_id)
	      	{
	      		if( (this.reply_comments[comment_id] != undefined) || (this.reply_attachment[comment_id] != undefined))
	      		{
		      		...
		      		axios.post(this.url+'/'+this.mode+'/post/reply/add/comment/'+comment_id,formData,{headers: {'Content-Type' : 'multipart/form-data'}}).then(response =>{
		      			this.success = response.data.success;
	      	...
	      	edit(id,type = null)
	      	{
	      		if(type == 'reply')
	      		{
		      		...
		      		axios.get(this.url+'/'+this.mode+'/post/edit/commentList/'+id).then(response => {
		    ...
		    closeModal()
		    {
	      	...
	      	editComment(comment_id)
	      	{
	      		...
	      		axios.post(this.url+'/'+this.mode+'/post/edit/comment/'+comment_id,formData,{headers: {'Content-Type' : 'multipart/form-data'}}).then(response =>{
	      			this.success = response.data.success;
	      			this.tab = false;
	      			this.showEdit = false;
	      			window.location.reload();
			...
			like(val,id)
			{
		        ...
		        axios.post(this.url+'/'+this.mode+'/post/comment/like/'+id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
		          	this.success = response.data.success;
		      		this.sendData(response.data.success);
		          	window.location.reload();
		        }).catch(error => {
			...
			dislike(val,id)
			{
		        ...
		        axios.post(this.url+'/'+this.mode+'/post/comment/dislike/'+id,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(response => {  
		          	this.success = response.data.success;
		      		this.sendData(response.data.success);
		          	window.location.reload();
		        }).catch(error => {
			...
			deleteComment(id)
			{
				var thisswal = this;
	      		swal({
			        ...
			        if (isConfirm) 
			        {
			          axios.get(thisswal.url+'/'+thisswal.mode+'/post/delete/comment/'+ id).then(response => {
        ...
        created() 
        {
            ...
            {
              	this.getData(this.url+'/'+this.mode+'/post/showList/'+this.id);
            }
